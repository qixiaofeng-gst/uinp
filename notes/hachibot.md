...
* 真机再次测试 ivector 方案的识别性能。
  * 讯飞的板子连续工作 20-30 分钟之间坏了 2 次。
  * 

2020/11/20 找回 ivector 方案的命中率。
1. 注册集是 279 条真机数据，测试集 89 条真机数据，测试集中 41 条纯噪音数据。
2. 原本纯噪音测试的命中率是 0.42，安静测试的命中率是 0.92，混合的是 0.58。
...
* 真机上的安静模式（只有风扇噪音和环境噪音）的命中率非常不稳定，从 90% 到 0% 之间任意波动。   
  初步找到了影响命中率的要素，识别率在一定情况下可控了。
* 踏步噪声带来的两个问题：唤醒词的命中率下降严重，声纹识别的命中率下降严重。   
  目前尝试了去噪和添加噪声训练的方案，对声纹识别的命中率没有提升。   
  唤醒词的命中率目前还没有靠谱的解决方案。

由于许多问题在团队现有的理解程度之下没有一个靠谱的解决方案，因此噪声环境下的命中率问题我们仍在学习和探索识别
（背景知识）
